...
// Prominent cross-link card between the two sites
...
// Top navigation bar with logo and opposite-site link
export function TopNav({ current, brandLabel, brandColor = '#FF4D8D' }) {
  // current: 'men' | 'asem'
  const otherHref = current === 'asem' ? '/' : '/asem';
  const otherLabel = current === 'asem' ? 'Для мужчин' : 'Для девушек';

  return (
    <nav style={{ padding: '18px 0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
      <Link href={current === 'asem' ? '/asem' : '/'} style={{ display: 'flex', alignItems: 'center', gap: 10, textDecoration: 'none', color: 'white' }}>
        <div style={{
          width: 38, height: 38, borderRadius: 12,
          background: current === 'asem' ? 'linear-gradient(135deg, #D4AF37, #FF4D8D)' : 'linear-gradient(135deg, #FF4D8D, #A855F7)',
          display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18,
        }}>{current === 'asem' ? '✨' : '🤍'}</div>
        <span style={{ fontWeight: 700, fontSize: 16, letterSpacing: current === 'asem' ? '0.05em' : '0' }}>
          {brandLabel}
        </span>
      </Link>
      <Link href={otherHref}
